#!/usr/bin/env python3
"""Binary irrigation classification (IRR vs NIRR) per day using 6 spectral features."""
...
DISPLAY_LABELS = {"NIRR": "Nao irrigado", "IRR": "Irrigado"}
IRR_LABEL = "IRR"
NIRR_LABEL = "NIRR"
...
def parse_args() -> argparse.Namespace:
    ...
    default_output_dir = project_dir / "outputs" / "tabelas" / "classificacao_irrigacao_por_dia"
    default_figure_dir = project_dir / "outputs" / "figuras" / "classificacao_irrigacao_por_dia"
    parser = argparse.ArgumentParser(
        description="Classificacao binaria IRR vs NIRR por dia usando bandas 1200, 1451, 1951, 970, 670 e NDRE."
    )
    parser.add_argument(
        "--input",
        type=Path,
...
def build_model_library() -> dict[str, Pipeline]:
    return {
        "Random Forest": Pipeline([
            ("imputer", SimpleImputer(strategy="median")),
            ("model", RandomForestClassifier(n_estimators=750, class_weight="balanced_subsample", random_state=42, n_jobs=-1)),
        ]),
        "SVM (RBF)": Pipeline([
            ("imputer", SimpleImputer(strategy="median")),
        ...
        "XGBoost": Pipeline([
            ("imputer", SimpleImputer(strategy="median")),
            ("model", XGBClassifier(objective="binary:logistic", eval_metric="logloss", n_estimators=400, max_depth=4, learning_rate=0.05, subsample=0.9, colsample_bytree=0.9, reg_lambda=1.0, random_state=42, n_jobs=1)),
        ]),
    }
...
def prepare_frame(df: pd.DataFrame) -> pd.DataFrame:
    ...
    frame["grupo_cv"] = frame["cultivar"].astype(str) + "_" + frame["condicao"].astype(str) + "_B" + frame["replicata"].astype(str)
...
def evaluate_single_model(pipeline: Pipeline, X: pd.DataFrame, y: np.ndarray, groups: np.ndarray, n_splits: int):
...
def run_classification(frame: pd.DataFrame, dia: str, features: list[str], output_dir: Path, figure_dir: Path) -> list[dict]:
    analysis_label = dia
    ...
    group_counts = frame.groupby("condicao").agg(n_amostras=("condicao", "size"), n_grupos=("grupo_cv", "nunique")).reset_index().sort_values("condicao")
    ...
    for model_name, pipeline in models.items():
        scores, y_pred, y_prob = evaluate_single_model(pipeline, X, y, groups, n_splits)
        ...
        predictions_df["analysis_id"] = dia
        ...
        confusion_df = pd.DataFrame(cm, index=[DISPLAY_LABELS[NIRR_LABEL], DISPLAY_LABELS[IRR_LABEL]], columns=[DISPLAY_LABELS[NIRR_LABEL], DISPLAY_LABELS[IRR_LABEL]]).reset_index(names="real")
        confusion_df.insert(0, "analysis_id", dia)
        confusion_df.insert(1, "analysis_label", analysis_label)
        confusion_df.insert(2, "dia", dia)
        confusion_df.insert(3, "modelo", model_name)

        sub_dir = output_dir / dia
        fig_sub_dir = figure_dir / dia
    ...
    write_csv(metrics_df, output_dir / dia / "metricas_todas_modelos.csv")
...
def main() -> None:
    ...
    print(f"\n{'='*80}")
    print("CLASSIFICACAO IRR vs NIRR - POR DIA (TODOS GENOTIPOS)")
    print(f"Features: {features}")
    print(f"{'='*80}\n")

    for dia in dias:
        subset = frame[frame["dia"] == dia].copy()

        if len(subset) < 4:
            print(f"[{dia}] Pulando - apenas {len(subset)} amostras")
            continue
        ...
        if n_irr < 2 or n_nirr < 2:
            print(f"[{dia}] Pulando - IRR={n_irr}, NIRR={n_nirr}")
            continue

        print(f"[{dia}] Amostras: {len(subset)} (IRR={n_irr}, NIRR={n_nirr})")

        results = run_classification(subset, dia, features, output_dir, figure_dir)
    ...
    lines = [
        "# Classificacao IRR vs NIRR - Por Dia (Todos Genotipos)\n",
        f"## Features: {', '.join(features)}\n",
        "| dia | n_amostras | melhor_modelo | accuracy | balanced_acc | f1_macro | roc_auc | kappa |",
        "| --- | ---: | --- | ---: | ---: | ---: | ---: | ---: |",
    ]
    for _, row in best_metrics_df.sort_values("dia").iterrows():
        lines.append(
            f"| {row['dia']} | {row['n_amostras']} | {row['modelo']} | "
